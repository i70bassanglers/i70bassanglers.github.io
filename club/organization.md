---
layout: layout.liquid
title: "Club Organization"
description: "Leadership structure, officers, and organizational details of I-70 Bass Anglers."
---

## President
**{{ club_officers.president.name }}**  
*Serving since {{ club_officers.president.year }}*
- **Responsibilities:** Overall club leadership, meeting coordination, external relations

## Vice President
**{{ club_officers.vice_president.name }}**  
*Serving since {{ club_officers.vice_president.year }}*
- **Responsibilities:** Member relations, event planning, president support

## Secretary
**{{ club_officers.secretary.name }}**  
*Serving since {{ club_officers.secretary.year }}* 
- **Responsibilities:** meeting minutes, membership records, website

## Treasurer
**{{ club_officers.treasurer.name }}**  
*Serving since {{ club_officers.treasurer.year }}*
- **Responsibilities:** Financial management, meeting minutes, membership records

## Tournament Director
**{{ club_officers.tournament_director.name }}**  
*Serving since {{ club_officers.tournament_director.year }}*
- **Responsibilities:** Tournament scheduling, rules enforcement, scoring

## Assistant Tournament Director
**{{ club_officers.asst_tournament_director.name }}**
*Serving since {{ club_officers.asst_tournament_director.year }}*
- **Responsibilities:** Tournament scheduling, rules enforcement, scoring